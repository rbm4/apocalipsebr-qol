<#
.SYNOPSIS
    Compiles patched VehicleManager.java and deploys the .class to Project Zomboid.

.DESCRIPTION
    S6 Optimization: Sparse vehicle connection state — iterate only active connections
    instead of all 512 slots in serverUpdate(). Reduces O(512*V) to O(P*V) per frame
    where P = active players and V = loaded vehicles.

    This script:
    1. Locates or downloads a JDK 25+ compiler (javac)
    2. Backs up the original VehicleManager.class from projectzomboid.jar
...
.NOTES
    ...
    The patch replaces an internal iteration pattern in VehicleManager.serverUpdate()
    with no change to external behavior or public API.
#>
param(
...
# --- Configuration ---
$PatchName     = "S6: Sparse Vehicle Connection State"
$SourceFile    = Join-Path $ToolsDir "zombie\vehicles\VehicleManager.java"
$GameJar       = Join-Path $PZDir "projectzomboid.jar"
$DeployDir     = Join-Path $PZDir "zombie\vehicles"
$DeployClass   = Join-Path $DeployDir "VehicleManager.class"
$BackupDir     = Join-Path $ToolsDir "backups"
$BackupClass   = Join-Path $BackupDir "VehicleManager.class.original"
$LocalJdkDir   = Join-Path $ToolsDir "jdk"
$OutputDir     = Join-Path $ToolsDir "out\classes"
$RequiredMajor = 25

# Also deploy the inner class PosUpdateVars
$InnerClassSuffix = 'VehicleManager$PosUpdateVars.class'
...
function Backup-OriginalClass {
    ...
    Write-Host "[*] Extracting original VehicleManager.class from JAR..." -ForegroundColor Cyan
    ...
    try {
        & jar xf $GameJar "zombie/vehicles/VehicleManager.class"
        if ($LASTEXITCODE -ne 0) {
            Write-Host "    ERROR: Failed to extract class from JAR." -ForegroundColor Red
            exit 1
        }

        $extracted = Join-Path $tempDir "zombie\vehicles\VehicleManager.class"
...
if ($Revert) {
    ...
    $innerClassPath = Join-Path $DeployDir $InnerClassSuffix
    if (Test-Path $innerClassPath) {
        Remove-Item $innerClassPath -Force
        Write-Host "Removed inner class: $innerClassPath" -ForegroundColor Green
        $reverted = $true
    }
    ...
    if ($reverted) {
        ...
        Write-Host "Original VehicleManager from JAR will be used on next server start." -ForegroundColor Gray
    } else {
...
# Step 3: Compile
Write-Host ""
Write-Host "[*] Compiling patched VehicleManager.java..." -ForegroundColor Cyan
...
$compiledClass = Join-Path $OutputDir "zombie\vehicles\VehicleManager.class"
...
if ($DryRun) {
    ...
    # List all compiled class files (main + inner classes)
    $compiledDir = Join-Path $OutputDir "zombie\vehicles"
    Get-ChildItem -Path $compiledDir -Filter "VehicleManager*.class" | ForEach-Object {
...
Write-Host ""
Write-Host "  The patched serverUpdate() iterates only active player connections" -ForegroundColor Gray
Write-Host "  instead of all 512 slots, reducing O(512*V) to O(P*V) per frame." -ForegroundColor Gray
Write-Host ""
Write-Host "  To revert entirely:" -ForegroundColor Yellow
Write-Host "    .\build-deploy-s6-sparse-vehicle-connections.ps1 -Revert" -ForegroundColor Yellow
Write-Host "    (or delete all VehicleManager*.class from: $DeployDir)" -ForegroundColor Yellow
Write-Host ""
