<#
.SYNOPSIS
    Compiles patched IsoChunkMap.java and deploys the .class to Project Zomboid.

.DESCRIPTION
    S9 Optimization: Reduce server chunkGridWidth to save ~170-350 MB RAM per player.

    The server doesn't need visual range — the default 13x13 (169 chunks) grid is
    designed for client-side rendering. On a dedicated server, reducing to 9x9 (81 chunks)
    eliminates 88 chunks per player, saving ~170-350 MB RAM per player.

    This script:
    1. Locates or downloads a JDK 25+ compiler (javac)
    2. Backs up the original IsoChunkMap.class from projectzomboid.jar
...
.NOTES
    ...
    The patch is controlled at runtime by a system property:
      -Dpz.server.chunk.grid.width=9    → 9x9 grid (default, saves ~170-350 MB per player)
      -Dpz.server.chunk.grid.width=13   → restore original 13x13 grid (disable optimization)
      Valid range: 5-19 (odd numbers only; even values rounded up)
#>
param(
...
# --- Configuration ---
$PatchName     = "S9: Reduce Server chunkGridWidth"
$SourceFile    = Join-Path $ToolsDir "zombie\iso\IsoChunkMap.java"
$GameJar       = Join-Path $PZDir "projectzomboid.jar"
$DeployDir     = Join-Path $PZDir "zombie\iso"
$DeployClass   = Join-Path $DeployDir "IsoChunkMap.class"
$BackupDir     = Join-Path $ToolsDir "backups"
$BackupClass   = Join-Path $BackupDir "IsoChunkMap.class.original"
...
function Backup-OriginalClass {
    ...
    Write-Host "[*] Extracting original IsoChunkMap.class from JAR..." -ForegroundColor Cyan
    ...
    try {
        & jar xf $GameJar "zombie/iso/IsoChunkMap.class"
        if ($LASTEXITCODE -ne 0) {
            Write-Host "    ERROR: Failed to extract class from JAR." -ForegroundColor Red
            exit 1
        }

        $extracted = Join-Path $tempDir "zombie\iso\IsoChunkMap.class"
...
if ($Revert) {
    if (Test-Path $DeployClass) {
        ...
        Write-Host "Original IsoChunkMap from JAR will be used on next server start." -ForegroundColor Gray
    } else {
...
# Step 3: Compile
Write-Host ""
Write-Host "[*] Compiling patched IsoChunkMap.java..." -ForegroundColor Cyan
...
$compiledClass = Join-Path $OutputDir "zombie\iso\IsoChunkMap.class"
...
Write-Host ""
Write-Host "  On a dedicated server, CalcChunkWidth() now sets chunkGridWidth to 9" -ForegroundColor Gray
Write-Host "  (81 chunks) instead of the default 13 (169 chunks)." -ForegroundColor Gray
Write-Host "  This saves ~170-350 MB RAM per connected player." -ForegroundColor Gray
Write-Host "  Client behavior is completely unchanged." -ForegroundColor Gray
Write-Host ""
Write-Host "  To customize the server chunk grid width, add to JVM args:" -ForegroundColor Yellow
Write-Host "    -Dpz.server.chunk.grid.width=7   (aggressive, 49 chunks)" -ForegroundColor Yellow
Write-Host "    -Dpz.server.chunk.grid.width=9   (default, 81 chunks)" -ForegroundColor Yellow
Write-Host "    -Dpz.server.chunk.grid.width=11  (conservative, 121 chunks)" -ForegroundColor Yellow
Write-Host "    -Dpz.server.chunk.grid.width=13  (original, disables optimization)" -ForegroundColor Yellow
Write-Host ""
Write-Host "  To revert entirely:" -ForegroundColor Yellow
Write-Host "    .\build-deploy-s9-chunk-grid.ps1 -Revert" -ForegroundColor Yellow
