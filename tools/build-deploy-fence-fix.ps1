<#
.SYNOPSIS
    Compiles patched ClimbOverFenceState.java and deploys the .class to Project Zomboid.

.DESCRIPTION
    This script:
    1. Locates or downloads a JDK 25+ compiler (javac)
    2. Extracts the original .class from projectzomboid.jar as a backup (first run only)
...
.NOTES
    ...
    Bug fixed: IsoZombie.tryThump() calls IsoDirections.fromAngle() which can return
    diagonal directions (NW, NE, SW, SE), but ClimbOverFenceState.setParams() only
    handles N, S, E, W — causing IllegalArgumentException and client disconnect.
    The patch snaps any direction to the nearest cardinal before the switch.
#>
param(
...
# --- Configuration ---
$ClassRelPath  = "zombie/ai/states/ClimbOverFenceState"
$SourceFile    = Join-Path $ToolsDir "$($ClassRelPath -replace '/', '\').java"
$GameJar       = Join-Path $PZDir "projectzomboid.jar"
$DeployDir     = Join-Path $PZDir ($ClassRelPath -replace '/[^/]+$' -replace '/', '\')
$DeployClass   = Join-Path $PZDir "$($ClassRelPath -replace '/', '\').class"
...
function Extract-OriginalClass {
    param(
        [string]$JarPath,
        [string]$EntryPath,
        [string]$OutputPath
    ...
    if (Test-Path $OutputPath) {
        Write-Host "    Original backup already exists: $OutputPath" -ForegroundColor Gray
        return
    }

    Write-Host "    Extracting original from JAR..." -ForegroundColor Gray
    ...
    try {
        ...
        if (-not $entry) {
            Write-Host "    WARNING: Entry '$EntryPath' not found in JAR. Skipping backup." -ForegroundColor Yellow
            return
        }
        ...
        Write-Host "    Backed up original: $OutputPath ($($entry.Length) bytes)" -ForegroundColor Green
    } finally {
        if ($zip) { $zip.Dispose() }
    }
}
...
# --- Main ---
Write-Host ""
Write-Host "=== ClimbOverFenceState Patch: Build & Deploy ===" -ForegroundColor White
Write-Host "    Fix: diagonal direction crash in setParams()" -ForegroundColor Gray
Write-Host ""
...
# Step 2: Extract original .class from JAR as backup
Write-Host ""
Write-Host "[*] Backing up original class from JAR..." -ForegroundColor Cyan
$jarEntryPath = "$($ClassRelPath).class"
$backupFile = Join-Path $BackupDir "$($ClassRelPath -replace '/', '\').class.original"
Extract-OriginalClass -JarPath $GameJar -EntryPath $jarEntryPath -OutputPath $backupFile
...
$compiledClass = Join-Path $OutputDir "$($ClassRelPath -replace '/', '\').class"
...
if ($DryRun) {
    ...
    if (Test-Path $DeployClass) {
        $backupOverride = "$DeployClass.bak.$(Get-Date -Format 'yyyyMMdd-HHmmss')"
        Copy-Item $DeployClass $backupOverride
        Write-Host "    Backed up existing override: $backupOverride" -ForegroundColor Gray
    }
...
Write-Host ""
Write-Host "The patched ClimbOverFenceState.class is deployed." -ForegroundColor Green
...
Write-Host ""
Write-Host "Original backup: $backupFile" -ForegroundColor Gray
Write-Host "To revert: delete $DeployClass (the JAR original will be used again)" -ForegroundColor Yellow
Write-Host "      or: copy the .class.original backup over the deployed file" -ForegroundColor Yellow
Write-Host ""
