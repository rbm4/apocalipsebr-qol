...
// PATCHED [S6]: Sparse vehicle connection state — iterate only active connections
...
public final class VehicleManager {
    ...
    // [S6 PATCH] Track only active connection indices instead of scanning all 512 slots.
    private final HashSet<Integer> activeConnectionIndices = new HashSet<>();
    private static boolean s6Logged = false;
    ...
    public void serverUpdate() {
        // [S6 PATCH] Log once on first call to confirm patch is active.
        if (!s6Logged) {
            s6Logged = true;
            System.out.println("[S6 PATCH] Sparse vehicle connection state active — iterating only active connections instead of 512 slots.");
        }

        ArrayList<BaseVehicle> vehicles = IsoWorld.instance.currentCell.getVehicles();

        // [S6 PATCH] Iterate only tracked active indices instead of all 512 slots.
        // Original: for (int i = 0; i < this.connected.length; i++) { ... }
        Iterator<Integer> it = this.activeConnectionIndices.iterator();
        while (it.hasNext()) {
            int i = it.next();
            if (this.connected[i] != null && !GameServer.udpEngine.connections.contains(this.connected[i])) {
                ...
                it.remove(); // [S6 PATCH] Remove from active set on disconnect.
            } else if (this.connected[i] != null) {
                for (int j = 0; j < vehicles.size(); j++) {
        ...
        if (this.updateRate.Check()) {
            ...
            for (int jx = 0; jx < GameServer.udpEngine.connections.size(); jx++) {
                ...
                this.connected[idx] = connection;
                this.activeConnectionIndices.add(idx); // [S6 PATCH] Track active index.
            }
