...
public final class BaseVehicle extends IsoMovingObject implements Thumpable, IFMODParameterUpdater, IPositional {
    ...
    // PATCH: Vehicle physics fix — smooth mass transitions to prevent flying
    private float lastFudgedMass = -1.0F;
    private static final float MASS_LERP_RATE = Float.parseFloat(System.getProperty("pz.vehicle.mass.lerp.rate", "200"));
    private static final float MAX_VERTICAL_FORCE = Float.parseFloat(System.getProperty("pz.vehicle.max.vertical.force", "5000"));
    private static final float MAX_VERTICAL_VELOCITY = Float.parseFloat(System.getProperty("pz.vehicle.max.vertical.velocity", "8"));
    ...
    private boolean disableSimulationDueToLackOfSurroundingChunks = false;
    private boolean desirePhysicsActive = false;
    private long ignoreCharacterCollisionSoundTimeout;
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        if (input.get() != 0) {
            ...
            try {
                ...
                DebugType.General.printException(var19, LogSeverity.Error);
            }
        ...
        if (!this.removedFromWorld) {
            if (!this.getCell().vehicles.contains(this)) {
                ...
                if (this.physics != null) {
                            ...
                            if (this.engineState != BaseVehicle.engineStateTypes.Idle) {
                                ...
                                WorldSoundManager.instance
                                    .addSoundRepeating(
                                        ...
                                        PZMath.fastfloor(this.getY()),
                                        PZMath.fastfloor(this.getZ()),
                                        Math.max(80, newEngineLoudness / 6),
                                        Math.max(6, newEngineLoudness / 3),
                                        (short)1
                                    );
                            }
                    ...
                    // PATCH: Post-collision vertical velocity sanity check.
                    // After all collision processing, if the vehicle has an abnormal
                    // upward velocity it means the physics solver produced an explosion.
                    // Clamp it to prevent the vehicle from flying into the sky.
                    // Configure via: -Dpz.vehicle.max.vertical.velocity=8
                    if (!GameServer.server && this.physics != null) {
                        Vector3f vel = this.getLinearVelocity(allocVector3f());
                        if (Math.abs(vel.y) > MAX_VERTICAL_VELOCITY) {
                            float yScale = vel.y == 0.0F ? 0.0F : MAX_VERTICAL_VELOCITY / Math.abs(vel.y);
                            DebugLog.Vehicle.warn("Vehicle vid=%d vertical velocity clamped: %.2f (scale=%.3f)",
                                this.vehicleId, vel.y, yScale);
                            Bullet.setVehicleVelocityMultiplier(this.vehicleId, 1.0F, yScale);
                        }
                        releaseVector3f(vel);
                    }
                }
                ...
                for (int ixxxx = 0; ixxxx < this.getScript().getPassengerCount(); ixxxx++) {
                    IsoGameCharacter var33 = this.getCharacter(ixxxx);
                    if (var33 instanceof IsoGameCharacter) {
                        Vector3f worldPos = this.getPassengerWorldPos(ixxxx, allocVector3f());
                        var33.setX(worldPos.x);
                        var33.setY(worldPos.y);
                        var33.setZ(worldPos.z);
    ...
    public void applyAccumulatedImpulsesFromHitObjectsToPhysics() {
        if (!this.impulsesFromHitObjects.isEmpty()) {
            if ((!GameClient.client || this.hasAuthorization(GameClient.connection)) && !GameServer.server) {
                ...
                // PATCH: Clamp vertical (Y) force component to prevent vehicles from
                // being launched into the air. The 30x FPS multiplier amplifies even
                // clamped forces significantly, so we cap the Y component separately.
                // Configure via: -Dpz.vehicle.max.vertical.force=5000
                float scaledForceY = force.y * 30.0F;
                if (Math.abs(scaledForceY) > MAX_VERTICAL_FORCE) {
                    scaledForceY = Math.signum(scaledForceY) * MAX_VERTICAL_FORCE;
                }

                float fpsScale = 30.0F;
                Bullet.applyCentralForceToVehicle(this.vehicleId, force.x * 30.0F, scaledForceY, force.z * 30.0F);
        ...
        if ((!GameClient.client || this.hasAuthorization(GameClient.connection)) && !GameServer.server) {
            ...
            if (hasImpulse) {
                ...
                if (force.lengthSquared() > 0.0F) {
                    ...
                    // PATCH: Clamp vertical force for prone character impulses too
                    float scaledForceY = force.y * 30.0F;
                    if (Math.abs(scaledForceY) > MAX_VERTICAL_FORCE) {
                        scaledForceY = Math.signum(scaledForceY) * MAX_VERTICAL_FORCE;
                    }
    ...
    // PATCH: Smooth mass transitions — lerp toward target instead of instant /3.7 jump.
    // The original code divides mass by 3.7 for towed non-trailer vehicles every frame,
    // causing Bullet physics to see a sudden mass change that amplifies forces and
    // launches vehicles into the air. We lerp toward the target mass at a configurable
    // rate (default 200 kg/frame) so the physics engine sees gradual transitions.
    // Configure via: -Dpz.vehicle.mass.lerp.rate=200
    public float getFudgedMass() {
        float targetMass;
        if (this.getScriptName().contains("Trailer")) {
            targetMass = this.getMass();
        } else {
            BaseVehicle vehicleA = this.getVehicleTowedBy();
            if (vehicleA != null && vehicleA.getDriver() != null && vehicleA.isEngineRunning()) {
                targetMass = Math.max(250.0F, vehicleA.getMass() / 3.7F);
                if (this.getScript().getWheelCount() == 0) {
                    targetMass = Math.min(targetMass, 200.0F);
                }
            } else {
                targetMass = this.getMass();
            }
        ...
        // First call or after reset: snap to target
        if (this.lastFudgedMass < 0.0F) {
            this.lastFudgedMass = targetMass;
            return targetMass;
        }

        // Lerp toward target at bounded rate
        float delta = targetMass - this.lastFudgedMass;
        if (Math.abs(delta) <= MASS_LERP_RATE) {
            this.lastFudgedMass = targetMass;
        } else {
            this.lastFudgedMass += Math.signum(delta) * MASS_LERP_RATE;
        }

        return this.lastFudgedMass;
    }
    ...
    private void applyDamageFromHitCharacters(VehiclePedestrianContactTracking pedestrianContacts) {
        ...
        if (totalDmgFront != 0 || totalDmgBack != 0) {
            if (GameClient.client) {
                GameClient.instance
                    .sendClientCommandV(null, "vehicle", "damageFromHitChr", "vehicle", this.getId(), "dmgFront", totalDmgFront, "dmgBack", totalDmgBack);
                return;
            }
    ...
    public float getCurrentSpeedKmHour() {
        return this.physics != null && !this.physics.isEnable ? (this.jniSpeed = 0.0F) : this.jniSpeed;
    }
                        ...
                        if (onGround) {
                            ...
                            if (!(IsoUtils.DistanceToSquared(wheelPos.x, wheelPos.y, closestX, closestY) > scriptWheel.radius * scriptWheel.radius)) {
                                if (doSound && currentAbsoluteSpeedKmHour > 10.0F) {
                                    ...
                                    } else if (!GameClient.client || this.checkIgnoreCollisionSoundCooldown()) {
    ...
    private void playSoundVehicleHitCharracter(IsoGameCharacter chr) {
        if (!GameServer.server) {
            if (!GameClient.client || this.checkIgnoreCollisionSoundCooldown()) {
    ...
    private void renderInterpolateBuffer() {
        if (this.netPlayerAuthorization == BaseVehicle.Authorization.Remote) {
            ...
            boolean ret = this.interpolation.interpolationDataGet(physicsBuf, engineSoundBuf, t - this.interpolation.delay);
            ...
            if (!GameServer.server) {
                ...
                for (int ix = 0; ix < this.newEngineSoundId.length; ix++) {
                    if (this.newEngineSoundId[ix] != 0L) {
                        this.getEmitter().setVolume(this.newEngineSoundId[ix], 1.0F - closestListenerDistSq / 1200.0F);
                    }
                }
            }
    ...
    public boolean checkIgnoreCollisionSoundCooldown() {
        long currentTime = System.currentTimeMillis();
        if (currentTime > this.ignoreCharacterCollisionSoundTimeout) {
            this.ignoreCharacterCollisionSoundTimeout = currentTime + 100L;
            return true;
        } else {
            return false;
        }
