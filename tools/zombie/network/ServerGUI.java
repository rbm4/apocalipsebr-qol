...
// PATCHED [S1]: Skip texture pack loading on server to save 50-200+ MB RAM.
// Texture loading is controlled by system property "pz.server.gui.textures":
//   -Dpz.server.gui.textures=true   → load all texture packs (original behavior)
//   (default)                        → skip texture packs, disable world rendering
...
public class ServerGUI {
    ...
    // [S1 PATCH] System property to control texture loading.
    // Default: false (skip textures). Set -Dpz.server.gui.textures=true to load them.
    private static final boolean loadTextures = Boolean.getBoolean("pz.server.gui.textures");
    ...
    public static void init() {
        ...
        try {
            ...
            // [S1 PATCH] Skip texture pack loading unless explicitly enabled.
            // Saves ~50-200+ MB RAM on the server when GUI mode is active.
            if (loadTextures) {
                ...
                System.out.println("[S1 PATCH] Skipping texture pack loading (saves ~50-200+ MB RAM).");
                System.out.println("[S1 PATCH] To enable textures, add -Dpz.server.gui.textures=true to JVM args.");
            }
    ...
    public static void init2() {
        if (created) {
            // [S1 PATCH] Skip vehicle texture loading unless explicitly enabled.
            if (loadTextures) {
                BaseVehicle.LoadAllVehicleTextures();
            } else {
                System.out.println("[S1 PATCH] Skipping vehicle texture loading.");
            }
        }
    }
    ...
    public static void update() {
        if (created) {
            ...
            // [S1 PATCH] When textures are not loaded, skip world rendering entirely
            // to avoid errors from missing texture data. Show a blank frame instead.
            if (!loadTextures || player == null) {
    ...
    private static void RenderTiles() {
        ...
        for (int zza = 0; zza <= maxZ; zza++) {
            ...
            for (int i = 0; i < SolidFloor.size(); i++) {
                IsoGridSquare squarex = SolidFloor.get(i);
                renderMinusFloor(squarex, false, false);
