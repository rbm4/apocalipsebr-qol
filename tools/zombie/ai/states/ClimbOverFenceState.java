...
// PATCHED: Snap diagonal directions to cardinal in setParams().
// IsoZombie.tryThump() can pass diagonal directions (NW, NE, SW, SE) via IsoDirections.fromAngle(),
// but setParams() only handles N, S, E, W — causing IllegalArgumentException and client disconnect.
// Fix: convert any direction to its nearest cardinal before the switch statement.
...
public final class ClimbOverFenceState extends State {
    ...
    public void setParams(IsoGameCharacter owner, IsoDirections dir) {
        // PATCHED: Snap diagonal directions to nearest cardinal (N, S, E, W).
        // IsoZombie.tryThump() uses IsoDirections.fromAngle() which can return diagonals (NW, NE, SW, SE),
        // but this method's switch only handles cardinal directions — causing IllegalArgumentException.
        dir = IsoDirections.cardinalFromAngle(dir.toAngle());
