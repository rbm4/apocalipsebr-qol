...
public final class CarController {
    ...
    public void update() {
        if (this.vehicleObject.getVehicleTowedBy() == null) {
            ...
            // PATCH: Only create constraint if one doesn't already exist.
            // The original code unconditionally calls addPointConstraint() every frame,
            // which breaks the existing constraint and creates a new one mid-physics step.
            // This momentarily frees both vehicles, causing momentum reversal and launch.
            if (vehicleTowedBy != null && vehicleTowedBy.getDriver() == null && this.vehicleObject.getDriver() != null && !GameClient.client
                && this.vehicleObject.constraintTowing == -1) {
