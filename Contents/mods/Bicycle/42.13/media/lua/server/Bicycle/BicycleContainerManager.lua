...
local function getBikeTrackingSquare(player, bikeItem)
    ...
    -- Mounted/in-hand bikes track to the rider's square so the world container
    -- follows the bike instead of getting packed into the player's inventory.
...
-- Cache the bike's container load (count + contents weight) onto the bike item's
-- own modData so the ride penalty can read it without hunting for the invisible
-- world containers. Those containers are server-authoritative and their position
-- is debounced (moved only when the rider changes square), so a client-side
-- positional lookup misses them in MP. Computing it here -- where the container
-- objects are reliably in hand regardless of where they currently sit -- and
-- transmitting it on the bike item gives every client a stable load value.
---@param bikeItem InventoryItem|nil
...
local function packPartContainerIntoContainer(player, bikeItem, def, containerInfo, srcContainer, destContainer)
    ...
    if containerItem and containerItem:getContainer() == srcContainer then
        ...
        -- The bike is already being committed to the destination, so it no longer has
        -- a world square. Pass the player so getTargetSquare can fall back to the
        -- rider's square: a mounted bike's hidden containers sit at the rider's feet,
        -- and this lets resolveDetachContainerWorldItem scan there if the cached
        -- world-item reference / coordinates ever go stale.
        local worldItem = resolveDetachContainerWorldItem(
            bikeItem,
            def.partType,
            def,
...
---Collect every bicycle a single player is carrying (inventory + both hands)
---into knownBikes. A mounted/ridden bike lives in its rider's own inventory and
---is never a world object, so it is invisible to any OTHER player's 3x3 world
---scan. That blind spot is what lets a second player's cleanup pass mistake a
---rider's still-attached hidden container for an orphan and spill it; scanning
---every player's carried bikes server-wide closes the gap.
---@param knownBikes table<number, InventoryItem>
---@param player IsoPlayer|nil
...
---Build the set of all bicycles carried by any online player (server) or any
---active player (SP). Used only to gate the destructive spill terminal in
---cleanup: a container whose owning bike is still carried by someone is not an
---orphan, so it must not be spilled. Relink/move decisions keep using the
---per-player local set (correct owner context), so this set never drives a
---cross-player relink.
...
local function cleanupHiddenContainerWorldItem(player, worldItem, def, knownBikes, claimedContainerKeys, globalKnownBikes)
    ...
    if claimedContainerKeys[key] == true then
        -- Duplicate world container for the same bike+part. Never destroy items here:
        -- the canonical was already reclaimed this tick. Leaving the duplicate alone
        -- lets normal sync logic resolve it once the bike state stabilizes.
        return
    end
    ...
    if bikeItem then
        -- Bike is around but the part isn't attached anymore. Drop the managed marker
        -- so this becomes a regular world container; items stay safe inside it.
        clearContainerMarker(worldItem)
        return
    end

    -- The bike is not in THIS player's inventory, hands, or 3x3 world, but a
    -- mounted/ridden bike lives in its rider's own inventory and is never a world
    -- object, so another player's local scan can never see it. If the owning bike
    -- is carried by any online player, this is a still-attached container trailing
    -- its rider (the owner's own pass will relink it), not an orphan -- leave it
    -- untouched instead of spilling the rider's items onto the ground.
    if bikeItemId and globalKnownBikes and globalKnownBikes[bikeItemId] then
        return
    end

    -- Bike is genuinely missing everywhere: inventory, hands, 3x3 world, and every
    -- online player's carry. The marker points at a bike we don't know about, so
    -- spill is the safe terminal.
...
local function onTick()
    ...
    -- Build the carried-bike set once per pass and share it with every tracked
    -- player so cleanup can tell a trailing-but-owned container from a true orphan
    -- without rescanning all inventories per player.
