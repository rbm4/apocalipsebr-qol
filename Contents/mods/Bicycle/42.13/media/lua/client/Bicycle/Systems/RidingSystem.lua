...
-- Stock m_SpeedScale of the deferred-rotation bicycle turn nodes. These MUST
-- match the AnimSets (turn_bicycle.xml = 0.80, turn_walk_bicycle.xml = 1.04).
-- The load penalty scales them down so the turn radius widens while riding;
-- this is the only lever that works at every pedal speed (see updateBicycleFlag).
...
function BicycleRidingSystem.updateBicycleAudio(player)
    ...
    if emitter:isPlaying("Bicycle_Riding") and player:isPlayerMoving() then
        ...
        if bicycleMenu then
            -- Pedals are NOT spun here: the crank is phase-locked to the pedal animation and driven every
            -- tick from updateBicycleFlag (see BicycleMenu.updatePedalCrank).
...
function BicycleRidingSystem.updateBicycleFlag(player)
    ...
    if BicycleUtils.isBicycleItem(primaryItem) then
        ...
        -- Phase-lock the pedal crank to the pedal animation. Local player only: the crank lives on the
        -- SHARED frame-model attachment, so letting remote riders drive it too would have them fight over
        -- one angle (the wheel spin has always had the same constraint).
        ...
        -- Turn-radius penalty uses two complementary levers so a loaded bike turns
        -- wider at any pedal speed without being penalized twice:
        --   * setTurnDelta (above) only bites while WALKING -- the engine returns
        --     the hardcoded turnDeltaRunning/turnDeltaSprinting once the rider runs
        --     or sprints, so it covers slow pedalling.
        --   * the deferred-rotation turn AnimSets (BicycleTurnSpeed / ...WalkSpeed)
        --     cover the FAST regime, where setTurnDelta is ignored.
        -- We gate the anim penalty to the fast regime so slow turns get the
        -- setTurnDelta penalty alone (no stacking). turnDelta is 1 when the penalty
        -- is off, restoring the stock 0.80 / 1.04 turn scales.
        ...
        -- Heavy load no longer locks the player to walking speed. The weight
        -- slowdown is carried by BicycleSpeed (the shared anim m_Scalar) which
        -- scales walk and run equally, so fast pedal stays proportionally
        -- faster than slow pedal (speedMultFast/speedMultSlow ratio preserved)
        -- while both remain slowed by the load.
        ...
        -- Zombie-contact slowdown: an extra multiplier driven by how many zombies are physically
        -- touching the rider (see Systems/ZombieProximity). Only the local rider is evaluated; a remote
        -- player's slowed speed arrives through the existing BicycleSyncClient state sync, so we must not
        -- recompute it from this client's view of the world.
        local zombieMult = 1
        ...
        -- finalSpeed (the weight / leg-damage / flat-tyre slowdown) is carried by the
        -- SpeedScale variables, NOT the BicycleSpeed scalar. The single-anim forward
        -- nodes (walk/run/sprint) honour both SpeedScale and Scalar, but the directional
        -- 2D-blend nodes (aim-strafe / strafe) only honour SpeedScale -- their deferred
        -- translation ignores m_Scalar. Keeping the slowdown in the scalar made the bike
        -- speed back up to unloaded levels while aiming/strafing (holding RMB to steer).
        -- EVERY speed slowdown (finalSpeed, zombieMult, enduranceScale) rides in the SpeedScale
        -- vars below and is applied ONCE there. The forward nodes honour SpeedScale * Scalar, so
        -- any slowdown ALSO left in this scalar would be SQUARED -- that was the 5.2.0 bug: zombieMult
        -- in both gave 2 contacts a forward of 0.36 instead of the designed 0.60 curve, and
        -- enduranceScale had the same latent double-apply. The 2D-blend (aim/strafe) nodes only honour
        -- SpeedScale anyway, so this scalar carries no slowdown at all: it is a plain 1.
