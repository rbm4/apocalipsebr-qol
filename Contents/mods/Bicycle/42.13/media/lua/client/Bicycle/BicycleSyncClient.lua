...
local function equipMountedBikeLocally(player, bikeItem)
    ...
    -- A ridden bike must show its kickstand up. The server flips it up in its mount handler but never
    -- tells the client: the mount path sends no SetKickstandVisual and that handler is a stub anyway, so
    -- the client keeps the parked-down kickstand it was normalized to during equip. Apply "up" locally to
    -- match the server before rebuilding the hand model. (Dismount re-transmits the whole world item,
    -- which is why only mounting showed the stale kickstand.)
...
Handlers[Core.SyncModule].MountEquip = function(args)
    ...
    -- AddInventoryItemToContainerPacket hasn't landed yet. Poll until it does.
    local maxTicks = 180
    local tickCount = 0
    local tick
    tick = function()
...
Handlers[Core.SyncModule].MountCancelled = function(args)
    ...
    -- The server has either undone a committed mount or rejected a pre-commit
    -- mount. Either way the bike no longer belongs in our hands. Local hands may
    -- already be empty (pre-commit cancel) — both calls are idempotent.
