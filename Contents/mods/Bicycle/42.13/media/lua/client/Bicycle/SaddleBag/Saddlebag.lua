...
function BicycleAttachments.onBikeDropped(player, bikeItem, dismountData)
    ...
    elseif isServer() then
        -- No server-side behavior in this client module.
    else
...
function BicycleAttachments.followPlayer(player, bikeItem)
    ...
    elseif isServer() then
        ...
        -- Singleplayer movement is owned by BicycleContainerManager's OnTick path.
        return
    end
end
...
function BicycleAttachments.measureContainerLoad(bikeItem)
    ...
    -- Prefer the server-cached load summary on the bike item. BicycleContainerManager
    -- computes it server/SP-side (where the invisible containers are reliably in hand)
    -- and transmits it on the bike, so it is correct on MP clients regardless of the
    -- debounced container positions. The positional lookup below is only a fallback for
    -- the brief window before the first sync (or pre-existing saves without the cache).
