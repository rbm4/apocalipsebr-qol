...
function BicycleStowAnimalAction:perform()
    ...
    -- LuaTimedActionNew gates `complete()` behind `!GameClient.client`, so on a
    -- multiplayer client (including the listen-server host's local client)
    -- complete() is never invoked. perform() runs on every side, so the
    -- server-bound dispatch lives here.
    local bikeItem = resolveBikeItem(self)
    if bikeItem then
        local payload = {
...
function BicycleStowAnimalAction:complete()
    -- Kept for SP / future server-side use; the dispatch already happened in
    -- perform() so this is just the success terminator.
    return true
end
