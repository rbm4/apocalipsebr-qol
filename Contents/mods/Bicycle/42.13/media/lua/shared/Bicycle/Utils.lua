...
function BicycleUtils.removeItemFromContainer(item)
    ...
    -- Vanilla MP ordering: container Remove → sendRemoveItemFromContainer (see
    -- ZomboidLua ISTransferAction:transferItem). Without this, server-side
    -- inventory removals never reach other clients and produce ghost duplicates.
    if isClient() then
