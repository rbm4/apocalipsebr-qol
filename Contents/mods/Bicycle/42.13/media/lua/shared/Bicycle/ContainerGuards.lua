...
function Bicycle_SidecarAcceptItem(container, item)
    ...
    -- Defense-in-depth: animals only enter the sidecar via the server bridge
    -- handler's direct AddItem call (which bypasses isItemAllowed). Block any
    -- drag-and-drop path explicitly so a future Java patch that tightens the
    -- AddItem path doesn't silently break this design.
