...
function Service.canDetachPart(character, bikeItem, partType)
    ...
    -- Animal weapon parts must be removed via "Release animal from sidecar" so
    -- the animal item in the sidecar container is removed in the same frame.
    if SidecarAnimal.isAnimalPartType(partType) then
        return false
    end
    -- The Sidecar is the prerequisite for the animal part; detaching it while
    -- occupied would orphan the animal-part link.
