...
-- Vanilla ships several loot-table variants of the plastic bag and the metal toolbox. They exist so that
-- different room types can spawn one pre-filled with different loot; otherwise they are the same item,
-- sharing a display name, an icon, a world model and a capacity. Every variant attaches to the bike and
-- every variant becomes the same bike part.
--
-- The FIRST entry in each list is canonical. It is what a detached part converts back into, and it is the
-- variant consumed first when the player is carrying several.
--
-- Base.Toolbox_Wooden is deliberately absent. It is a different item: its own icon (ToolBox_Old), its own
-- world model, six slots instead of eight, and the bike's toolbox mesh is the metal one.
...
-- PlasticBagLeft and PlasticBagRight share one list, so a single membership test covers both sides.
...
---The canonical vanilla item for a part type. A detached part converts back into this one, whichever
---variant was originally attached.
---@param partType string|nil
---@return string|nil
---@nodiscard
...
---Every vanilla item accepted as a source for a part type, canonical first.
---@param partType string|nil
---@return string[]|nil
---@nodiscard
...
---Find a vanilla item in `inventory` that is accepted as a source for `partType`. Variants are searched
---canonical first, so a plain Plastic Bag is consumed before a loot-table variant when both are carried.
