...
function BicycleReleaseAnimalAction:perform()
    -- LuaTimedActionNew skips Lua complete() on MP clients (GameClient.client
    -- guard). Dispatch from perform() instead, which always runs client-side.
    local bikeItem = resolveBikeItem(self)
    if bikeItem then
...
function BicycleReleaseAnimalAction:complete()
    -- Dispatch already happened in perform(); this is just the success terminator.
    return true
end
