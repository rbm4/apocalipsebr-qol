...
function BicycleHopOnAction:stop()
    ...
    -- Local visual cleanup: clear any bike from hands, reset mount visuals.
    local bikeItem = resolveBikeItem(self)
    if bikeItem then
    ...
    if isClient() then
        -- The server is authoritative. Tell it the mount was cancelled.
        -- - If :complete() hasn't run yet, the server marks a pending-cancel
        --   flag which :complete() consults and bails on.
        -- - If :complete() already committed, the server undoes the mount
        --   (remove from inventory, place back at sourceSquare) and broadcasts
        --   a MountCancelled command for local cleanup confirmation.
    ...
    elseif isServer() then
        -- :stop() does not run server-side for NetTimedAction: when the client
        -- sends Reject, NetTimedAction.stop calls serverStop (not defined here).
        -- Cancel cleanup is owned by the CancelMount Bridge handler.
    else
        -- Singleplayer: BicycleMenu.equipBicycle queues
        -- ISInventoryTransferAction + ISEquipWeaponAction before this hop-on
        -- action, so by the time the user cancels, the bike has already been
        -- moved from the ground into the player's inventory. Undo that move
        -- here so the bike returns to its original square with its original
        -- rotation. This mirrors the MP CancelMount/undoCommittedMount path.
...
function BicycleHopOnAction:perform()
    ...
    elseif isServer() then
        -- Authoritative mutation runs in :complete() on the server.
    else
        -- Singleplayer authoritative mutation runs in :complete().
    end
...
function BicycleHopOnAction:complete()
    ...
    -- If the player cancelled the mount but the cancel arrived after the action
    -- had already finished its duration server-side, the CancelMount handler set
    -- a flag. Consume it and bail out before committing the world→inventory
    -- transfer; the action will be reported as Reject to the client.
    ...
    if isClient() then
        -- :complete() does not run client-side in MP (LuaTimedActionNew.java:165).
    elseif isServer() then
        -- Server-side setPrimaryHandItem auto-broadcasts an EquipPacket via
        -- preupdate (handItemShouldSendToClients flag), which races
        -- AddInventoryItemToContainerPacket on a different channel. Defer the
        -- equip to the owning client via MountEquip; client equips locally only
        -- once it observes the bike in inventory, eliminating the race.
