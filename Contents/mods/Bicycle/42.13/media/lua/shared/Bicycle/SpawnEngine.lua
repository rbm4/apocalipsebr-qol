-- Stateless world-spawn engine. Replaces the old "spawn on every chunk load, suppress with a
-- remembered claim" design, whose claim tables lived in global ModData (lost on an unclean server
-- shutdown) and whose per-square fallback depended on chunk load order (arbitrary in MP).
--
-- Nothing is persisted. A spawn happens at most once per building because:
--   * the roll is a pure hash of the building's immutable map bounding box, and
--   * exactly one chunk in the world -- the one with the largest overlap of the winning room -- is
--     allowed to place, and it may only place into its own squares.
-- The engine itself never runs on an MP client (guarded), and IsoChunk.isNewChunk() is false on a
-- client by construction (IsoChunk.LoadBrandNew gates addZombies on !GameClient.client), so a client
-- cannot spawn even if the guard were bypassed.
...
-- Per-save salt so two worlds on the same map do not place items identically.
---@return number
---@nodiscard
...
-- Best-effort: only squares whose chunk is currently loaded can be inspected. Its job is to notice an
-- item an older mod version already placed elsewhere in the building; the ownership rule below is what
-- guarantees this engine never places twice.
...
-- The chunk covering the largest part of the room owns every placement decision for it. Ties resolve
-- by first-encountered in a fixed rect/chunk scan order, so the answer is identical in every session.
...
local function collectTargetRooms(spec, roomDef, buildingDef)
    ...
    -- getRooms() order comes from metagrid load order; sort so the winner pick below cannot drift.
...
local function computeDecision(spec, roomDef, buildingDef)
    ...
    -- Probability that at least one target room would have hit under the old per-room roll, so the
    -- shipped per-room tuning keeps its meaning now that a building rolls once.
    local missAll = 1
    local weightTotal = 0
...
-- Retrofit for worlds whose chunks were generated before this mod (or this version) was installed.
-- Both flags persist in map_meta and are set the moment a player sees or enters the place, so the
-- window closes before the item could have been taken -- the re-check always finds its own spawn.
...
-- The two -- and only two -- doors through which a placement can happen. Both the live LoadGridsquare
-- path and SpawnEngine.probePlacement go through here, so a test that asks the gate is asking the
-- same code the game runs, not a copy of it.
---@param decision table
---@param buildingDef BuildingDef|nil
---@param chunkX number
---@param chunkY number
...
-- Pure: the spawn decision for a room's building. Depends only on map geometry and the save salt, so
-- it returns the same answer in every session, on the server and in SP, without loading a single chunk.
---@param id string
...
-- Read-only: would the live gate place here, right now? Deliberately reports `resolvedThisSession`
-- separately instead of short-circuiting on it, because that flag is memory-only and dies with the
-- session. The guarantee a test needs to prove is that the two PERSISTED doors are shut.
---@param id string
---@param roomDef RoomDef|nil
---@return table|nil
---@nodiscard
