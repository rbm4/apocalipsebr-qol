    ...
    for luaItArea, area in ipairs(areaOrder) do
        for luaItMb,partItMb in ipairs(jobList) do
            ...
            if part:getArea() == area then
                ...
                if AutoMechanics.getVerbose() then print ("sortMechanicJobListByPosition : "..iterOrdered.." "..tostring(part:getItemType() or "nil").." "..part:getArea()) end
            end
...
function AutoMechanics.doPendingJob(player,vehicle)
                ...
                if alreadyUninstalled then--that's the time to do a dependency search
                    ...
                    for luaItMb,partItMb in ipairs(jobOrganisation.jobListMaybe) do
                        ...
                        if jobAvailability == 2 then
                            ...
                            if AutoMechanics.getVerbose() then print ("AutoMechanics.doPendingJob Activate and Uninstall maybe part "..partIt);end
                            break
                        end
                    end
                    if not maybeJobAvailable then
                        if AutoMechanics.getVerbose() then print ("AutoMechanics.doPendingJob Install part "..partIt);end
                        doInstall = true;--for now we just do install
                    end
                else
                    if AutoMechanics.getVerbose() then print ("AutoMechanics.doPendingJob Uninstall part "..partIt);end
    ...
    if doInstall then
        ...
        if not AutoMechanics.uninstallOnly and item and isInstallStillPossible then
            ...
            if AutoMechanics.getVerbose() then print ("AutoMechanics.doPendingJob Install part it "..partIdToDo.." on area "..tostring(targetArea)..tostring(sameArea and " unchanged" or " changed") );end
            ...
            if AutoMechanics.getVerbose() and not isInstallStillPossible then print ("AutoMechanics.doPendingJob job Install not possible, missing access to the part for it "..partIdToDo); end
            if AutoMechanics.getVerbose() and isInstallStillPossible and not AutoMechanics.uninstallOnly then print ("AutoMechanics.doPendingJob job Install not possible, missing valid item for part it "..partIdToDo); end
            if AutoMechanics.getVerbose() and AutoMechanics.uninstallOnly then print ("AutoMechanics.doPendingJob job not reinstalling as requested by Deadly_Shadow "..partIdToDo); end
        ...
        if AutoMechanics.getVerbose() then print ("AutoMechanics.doPendingJob Uninstall part it "..partIdToDo);end
