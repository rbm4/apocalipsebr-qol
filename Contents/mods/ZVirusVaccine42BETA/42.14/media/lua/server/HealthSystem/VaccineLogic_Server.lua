...
--[[
---------------------------------
-- NOVA FUNCIONALIDADE EXPERIMENTAL: IMUNIDADE POR 24 HORAS APÓS A CURA.
---------------------------------
...
local CURE_IMMUNITY_HOURS = 24

local function IsImmune(player)
    local pMod = player:getModData()
    if not pMod or not pMod.CureImmunityUntil then return false end
    return player:getHoursSurvived() < pMod.CureImmunityUntil
end

function VaccineLogic.IsImmune(player)
    return IsImmune(player)
end

local function CurePlayer(body, player, grantImmunity)
    if not body or not player then return false end
    ...
    -- Imunidade pós-cura: concedida apenas quando usada em jogador infectado
    if grantImmunity then
        pMod.CureImmunityUntil = player:getHoursSurvived() + CURE_IMMUNITY_HOURS
    end
...
local function InfectPlayerAccidental(body, player)
    -- Jogador imune não pode ser infectado acidentalmente
    if IsImmune(player) then return end
...
function VaccineLogic.ProcessInjection(player, itemType)
    ...
    if isInfected then
        ...
        if vaccine.CureChance > roll then
            -- grantImmunity=true apenas para a cura definitiva usada em infectado
            local grantImmunity = (vaccine.Quality == 4)
            CurePlayer(body, player, grantImmunity)
            return
        end
    ...
    if vaccine.InfectChance > ZombRand(100) and not IsImmune(player) then
        InfectPlayerAccidental(body, player)
        return
    end
