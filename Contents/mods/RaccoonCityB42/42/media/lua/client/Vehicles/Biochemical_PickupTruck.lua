local function info()
    ...
    ISCarMechanicsOverlay.CarList["Base.Biochemical_PickupTruck"] = {
        imgPrefix = "Biochemical_PickupTruck_",
        x = 10,
        y = 0
    };
...
local function onEnterBiochemicalTruck(player)
    ...
    if player:getPlayerNum() < 0 then
        return
    end -- skip remote players
    local vehicle = player:getVehicle()
    if not vehicle then
        return
    end
    if vehicle:getScriptName() ~= "Base.Biochemical_PickupTruck" then
        return
    end
