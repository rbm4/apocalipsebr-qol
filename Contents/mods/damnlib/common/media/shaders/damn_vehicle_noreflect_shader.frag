...
void main()
{
    ...
    texColorRust.rgb = texColorRust.rgb * vec3(1.1, 0.7, 0.5);
    ...
    col = col*lighting*TintColourNew * 1.1;
    ...
    col = mix(col, texRef, ref_en*(0.1+ReflectionParam.y*0.1)); // 0.6 max 0.1 min
    col = mix(col, texRef/4.0, (1.0-ref_en)*(0.05 + ReflectionParam.z*0.15)); // 0.2 max 
    ...
    col = addBlood(texColorBlood2 * vec4(lighting, 1.0), colmask, intensity, maskAlpha * windowAlpha, lighting, TintColourNew, col);
    ...
    col = addBlood(texColorBlood2 * vec4(lighting, 1.0), colmask, intensity, maskAlpha * (1.0 - windowAlpha), lighting, TintColourNew, col);
