...
function DAMN.inVehicleUpdateTask(player)
    if DAMN["currentVehicleScript"] and DAMN.Armor["byVehicleScript"][DAMN["currentVehicleScript"]] then
        DAMN.Armor:runArmorCode(player, DAMN["currentVehicle"]);
        DAMN.ArmorProtection.apply(DAMN["currentVehicle"]);

        local currentTime = Calendar.getInstance():getTimeInMillis();

        if currentTime > DAMN.Armor["lastPartUpdate"] + DAMN.Armor["partUpdateInterval"] then
            DAMN.Armor:flushPartsBuffer(DAMN["currentVehicle"], currentTime);
        end
    end

    if DAMN["currentVehicle"]:isEngineRunning() then
        if DAMN.Armor:hasMagicTires() then
            DAMN.Armor:updateMagicTires(DAMN["currentVehicle"]);
        end

        if DAMN["vehicleHasAirBrake"] then
            local speed = DAMN["currentVehicle"]:getCurrentSpeedKmHour();

            if speed == 0 and not DAMN["airBrakePlayed"] then
                local emitter = getPlayer():getEmitter();

                if not emitter:isPlaying("damnBrake") then
            ...
            if speed ~= 0 and DAMN["airBrakePlayed"] then
...
Events.OnEnterVehicle.Add(function(player)
    ...
    if vehicle then
        ...
        if DAMN.Armor:updateEventRequired() then
            ...
            if DAMN["armorEventDebug"] and DAMN.Armor:hasMagicTires() then
                DAMN:log(" -> rfs/ctis exists");
            end

            if DAMN["armorEventDebug"] and DAMN["vehicleHasAirBrake"] then
...
Events.OnServerCommand.Add(function(moduleName, command, args)
    if moduleName == "that_damn_lib" then
        if command == "playPartAnimation" and args["vehicleId"] and args["animation"] then
            ...
            if vehicle then
                ...
                if part then
                    ...
                    if args["animation"] == "Close" or args["animation"] == "Open" then
                        ...
                        if seat then
                            vehicle:playPassengerAnim(seat, args["animation"] == "Open" and "openDoor" or "closeDoor",
                                player);
                        else
