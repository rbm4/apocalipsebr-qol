            ...
            for _, option in ipairs(options.data or {}) do
                ...
                elseif option.type == "multipletickbox" then
                    ...
                    for i, value in ipairs(option.values) do
                        local t = labelText(value.name)
                        multipleTickBox:addOption(t, value.name)
                        optTextByIdx[i] = t
                    end
                    self.addY = self.addY + multipleTickBox:getHeight() + 4
                    for i, v in ipairs(option.values) do
                        if v.isEnabled == false then
                            multipleTickBox:disableOption(optTextByIdx[i], true)
                        end
                    end
                ...
                elseif option.type == "combobox" then
                    ...
                    local combo = self:addCombo(splitpoint, y, comboWidth, 20, oname, values, option.selected or 1)
                    ...
                    function go:toUI()
                        self.control.selected = option.selected or 1
                    end
                    function go:apply()
                        local sel = self.control.selected
                    ...
                    function go:onChange(box)
                        if option.onChange then option:onChange(box.selected) end
                    end
                    self.gameOptions:add(go)
